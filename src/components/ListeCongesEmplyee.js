...
export default function ListeCongesEmployee() {
    ...
    const DeleteConges = (id) => {
        axios.delete(`http://localhost:8082/conges/api/remove-conges/${id}`)
        .then(()=>{
            axios.get("http://localhost:8082/conges/api/retrieve-all-conges")
            .then((getData)=>{
                setListConges(getData.data);
                                ...
                                <tbody>
                                    
                                    {
                                        listConges.map((e,key)=>(
                                            ...
                                            <td>{e.etat}</td>
                                            <td>
                                                <button className="btn btn-danger" onClick={()=>DeleteConges(e.idConges)} >delete</button></td>
                                            </tr>
                                        ))
                                    }
                                </tbody>
                            </table>
                            <a href="/HR/congesEmployee/demand" style={{color: "white"}}>
